...
from zmx_after_raw_data import *
...
def biGRU(data, cell_fw, cell_bw, scope1, scope2, reused):
    ...
    with tf.variable_scope(scope2, reuse=reused):
        output_bw, state_bw = tf.nn.dynamic_rnn(cell_bw, reverse_data, dtype=tf.float32,
                                                sequence_length=length(reverse_data))
...
def main():
    ...
    with graph.as_default(), tf.device('/gpu:1'):
        print_checkers = []
        QA_NL_character_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                       params.qa_nl_character_max_length])
        QA_NL_word_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                  params.qa_nl_word_max_length])
        QA_NL_phrase_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                    params.qa_nl_phrase_max_length])
        QA_KB_character_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                       params.qa_kb_character_max_length])
        ...
        QA_KB_neg_character_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_neg_size,
                                                                           params.qa_kb_character_max_length])
        QA_KB_neg_word_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_neg_size, params.qa_kb_word_max_length])
        QA_KB_neg_phrase_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_neg_size,
                                                                        params.qa_kb_phrase_max_length])

        QA_NL_oov_word_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                      params.qa_nl_word_max_length])
        QA_NL_oov_phrase_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                        params.qa_nl_phrase_max_length])

        QA_KB_oov_word_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                      params.qa_kb_word_max_length])
        QA_KB_oov_phrase_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_batch_size,
                                                                        params.qa_kb_phrase_max_length])

        QA_KB_neg_oov_word_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_neg_size,
                                                                          params.qa_kb_word_max_length])
        QA_KB_neg_oov_phrase_train_inputs = tf.placeholder(tf.int32, shape=[params.qa_neg_size,
                                                                            params.qa_kb_phrase_max_length])
        ...
        QA_c2w_tree = tf.placeholder(tf.float32, shape=[params.sf_batch_size, params.qa_nl_word_max_length,
                                                        params.qa_nl_character_max_length])
        # checkers .append(tf.check_numerics(QA_c2w_tree, "nan error 1"))

        QA_w2p_tree = tf.placeholder(tf.float32, shape=[params.sf_batch_size, params.qa_nl_phrase_max_length,
                                                        params.qa_nl_word_max_length])
        ...
        QA_NL_character_mask = tf.placeholder(tf.float32,
                                              shape=[params.qa_batch_size, params.qa_nl_character_max_length])
        ...
        QA_KB_character_mask = tf.placeholder(tf.float32,
                                              shape=[params.qa_batch_size, params.qa_kb_character_max_length])
        ...
        QA_KB_neg_character_mask = tf.placeholder(tf.float32,
                                                  shape=[params.qa_neg_size, params.qa_kb_character_max_length])
        ...
        with tf.variable_scope('QA_character'):
            ...
            # QA_character_weight = tf.Variable(tf.truncated_normal([(params.character_hidden_size +
            # params.character_hidden_size)*2, params.character_hidden_size], -1.0, 1.0))
            # QA_character_bias = tf.Variable(tf.zeros([params.character_hidden_size]))

            # QA_character_pro = tf.Variable(tf.truncated_normal([params.character_hidden_size, 1], -1.0, 1.0))
            # # matmul要求y矩阵必须是二维

            QA_character_weight = tf.Variable(tf.truncated_normal([(params.character_hidden_size +
                                                                    params.character_hidden_size)*2, 1], -1.0, 1.0))
            QA_character_bias = tf.Variable(tf.zeros([1]))

        with tf.variable_scope('QA_word'):
            # QA_word_weight = tf.Variable(tf.truncated_normal([(params.word_hidden_size+params.word_hidden_size)*2,
            # params.word_hidden_size], -1.0, 1.0))
            ...
            QA_word_weight = tf.Variable(tf.truncated_normal([(params.word_hidden_size+params.word_hidden_size)*2, 1],
                                                             -1.0, 1.0))
            QA_word_bias = tf.Variable(tf.zeros([1]))

        with tf.variable_scope('QA_phrase'):
            # QA_phrase_weight = tf.variable(tf.truncated_normal([(params.phrase_hidden_size +
            # params.phrase_hidden_size)*2, params.phrase_hidden_size], -1.0, 1.0))
            ...
            QA_phrase_weight = tf.Variable(tf.truncated_normal([(params.phrase_hidden_size+params.phrase_hidden_size) *
                                                                2, 1], -1.0, 1.0))
        ...
        with tf.name_scope("embeddings_initializer_NL_charater"):
            NL_character_embeddings = embeddings_initializer(loader.nl_character_oov_begin-1,
                                                             params.character_hidden_size, -1.0, 1.0)
        ...
        with tf.name_scope("embeddings_initializer_NL_phrase"):
            NL_phrase_embeddings = embeddings_initializer(loader.nl_phrase_oov_begin-1, params.phrase_hidden_size,
                                                          -1.0, 1.0)
        ...
        with tf.name_scope("embeddings_initializer_KB_phrase"):
            KB_phrase_embeddings = embeddings_initializer(loader.kb_phrase_oov_begin-1, params.phrase_hidden_size,
                                                          -1.0, 1.0)
        ...
        with tf.name_scope("get_oov_embedding"):
            NL_character_of_oov_word_embedding = tf.reshape(tf.nn.embedding_lookup(NL_character_embeddings,
                                                                                   tf.reshape(NL_character_of_oov_word,
                                                                                              [-1])),
                                                            [-1, params.word_max_length, params.character_hidden_size])
            # print_checkers.append(NL_character_of_oov_word_embedding)
            with tf.variable_scope('NL' + 'c2w'):
                NL_oov_word_embedding = tf.concat(0, [tf.zeros([1, params.character_hidden_size], tf.float32),
                                                      tf.nn.dynamic_rnn(NL_c2w_cell, NL_character_of_oov_word_embedding,
                                                                        dtype=tf.float32,
                                                                        sequence_length=
                                                                        length(NL_character_of_oov_word_embedding))[1]])
                # print_checkers.append(NL_oov_word_embedding)
                oov_embedding_checker = tf.check_numerics(NL_oov_word_embedding, "nan error 17")

            NL_word_of_oov_phrase_embedding = tf.reshape(tf.nn.embedding_lookup(NL_word_embeddings,
                                                                                tf.reshape(NL_word_of_oov_phrase,
                                                                                           [-1])),
                                                         [-1, params.phrase_max_length, params.word_hidden_size])
            NL_oov_word_of_oov_phrase_embedding = tf.reshape(tf.nn.embedding_lookup(NL_oov_word_embedding,
                                                                                    tf.reshape(
            ...
            with tf.variable_scope('NL' + 'w2p'):
                NL_oov_phrase_embedding = tf.concat(0, [tf.zeros([1, params.word_hidden_size],
                                                                 tf.float32),
                                                        tf.nn.dynamic_rnn(NL_w2p_cell,
                                                                          NL_word_of_oov_phrase_embedding_complete,
                                                                          dtype=tf.float32,
                                                                          sequence_length=
                                                                          length(NL_word_of_oov_phrase_embedding_complete))[1]])
