...
def main():
	...
	with graph.as_default():
		...
		with tf.name_scope("get_oov_embedding"):
			...
			# print_checkers.append(KB_oov_word_embedding)
			# print_checkers.append(KB_oov_word_of_oov_phrase)
	...
	with tf.Session(graph=graph, config = tf.ConfigProto(allow_soft_placement = True, log_device_placement = False)) as sess:
		...
		for e in range(params.epoch_num):
			...
			for b in range(loader.qa_batch_num):
				...
				print "len(res[5]):", len(res[5])

				with open("norm_info", "a") as f:
					print >>f, "batch {0}:".format(b)
