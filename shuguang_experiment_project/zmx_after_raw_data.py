		...
		for k, v in oov_phrase_index_sorted:
			...
			for x in range(len(ins)):
				if oov_word_index.has_key(ins[x]):
                                        ...
                                        # 以下代码有问题，这回导致oov_phrase的oov_word和非oov_word对应位的值不对应
			...
			if(len(new_ins) > params.phrase_max_length):
				new_ins = ins[0:params.phrase_max_length]
...
#填补所有placeholder
	def next_batch(self):
		...
		'''
                ...
                for i in range(10):
                    for j in range(20):
                        print QA_NL_word_train_inputs[i][j], "\t", QA_NL_oov_word_train_inputs[i][j], "\t", QA_KB_word_train_inputs[i][j], "\t",QA_KB_oov_word_train_inputs[i][j],"\t", QA_KB_neg_word_train_inputs[i][j],"\t", QA_KB_neg_oov_word_train_inputs[i][j]
                """
...
if __name__ == '__main__':
	...
	loader.reset_batch_pointer()
        for i in range(316):
            ...
            if i == 315:
                for j in range(10):
                    print "QA_NL_phrase_train_inputs", data[2][j]
                    print "QA_KB_phrase_train_inputs", data[5][j]
                    print "QA_KB_neg_phrase_train_inputs", data[8][j]
                    print "QA_NL_oov_phrase_train_inputs", data[10][j]
                    print "QA_KB_oov_phrase_train_inputs", data[12][j]
                    print "QA_KB_neg_oov_phrase_train_inputs", data[14][j]
                    print "QA_NL_phrase_mask", data[25][j]
                    print "QA_KB_phrase_mask", data[28][j]
                    print "QA_KB_neg_phrase_mask", data[31][j]
                    print "\n" * 3
