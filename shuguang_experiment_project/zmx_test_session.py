...
def main():
	...
	with graph.as_default():
		...
		with tf.variable_scope('QA_character', reuse = True):
                            # QA_neg_character_unnormalized_score =tf.exp(tf.reshape(tf.matmul(tf.tanh(tf.matmul(QA_neg_concat_character_state_temp, QA_character_weight)+QA_character_bias), QA_character_pro),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_character_max_length, params.qa_kb_character_max_length]))
                            # 将数据限制在较小的范围内！将tf.tanh()换成tf.sigmoid(),再于最外围套上一个tf.sigmoid()
			# QA_neg_character_unnormalized_score = tf.sigmoid(tf.exp(tf.reshape(tf.matmul(tf.sigmoid(tf.matmul(QA_neg_concat_character_state_temp, QA_character_weight)+QA_character_bias),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_character_max_length, params.qa_kb_character_max_length])))
		...
		with tf.variable_scope('QA_word'):
                            # QA_word_unnormalized_score = tf.exp(tf.reshape(tf.matmul(tf.tanh(tf.matmul(QA_concat_word_state_temp, QA_word_weight)+QA_word_bias), QA_word_pro),[params.qa_batch_size, params.qa_nl_word_max_length, params.qa_kb_word_max_length]))
                            # 将数据限制在较小的范围内！将tf.tanh()换成tf.sigmoid(),再于最外围套上一个tf.sigmoid()
			# QA_word_unnormalized_score = tf.sigmoid(tf.exp(tf.reshape(tf.matmul(tf.sigmoid(tf.matmul(QA_concat_word_state_temp, QA_word_weight)+QA_word_bias), QA_word_pro),[params.qa_batch_size, params.qa_nl_word_max_length, params.qa_kb_word_max_length])))
		...
		with tf.variable_scope('QA_word', reuse = True):
                            # QA_neg_word_unnormalized_score = tf.exp(tf.reshape(tf.matmul(tf.tanh(tf.matmul(QA_neg_concat_word_state_temp, QA_word_weight)+QA_word_bias), QA_word_pro),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_word_max_length, params.qa_kb_word_max_length]))
                            # 将数据限制在较小的范围内！将tf.tanh()换成tf.sigmoid(),再于最外围套上一个tf.sigmoid()
			# QA_neg_word_unnormalized_score = tf.sigmoid(tf.exp(tf.reshape(tf.matmul(tf.sigmoid(tf.matmul(QA_neg_concat_word_state_temp, QA_word_weight)+QA_word_bias), QA_word_pro),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_word_max_length, params.qa_kb_word_max_length])))
		...
		with tf.variable_scope('QA_phrase'):
                            # QA_phrase_unnormalized_score = tf.exp(tf.reshape(tf.matmul(tf.tanh(tf.matmul(QA_concat_phrase_state_temp, QA_phrase_weight)+QA_phrase_bias), QA_phrase_pro),[params.qa_batch_size, params.qa_nl_phrase_max_length, params.qa_kb_phrase_max_length]))
                            # 将数据限制在较小的范围内！将tf.tanh()换成tf.sigmoid(),再于最外围套上一个tf.sigmoid()
                            # QA_concat_phrase_Wx_add_b = tf.matmul(QA_concat_phrase_state_temp, QA_phrase_weight)+QA_phrase_bias
                            # # checkers .append(tf.check_numerics(QA_concat_phrase_Wx_add_b, "nan error 35"))

			# QA_phrase_unnormalized_score = tf.sigmoid(tf.exp(tf.reshape(tf.matmul(tf.sigmoid(QA_concat_phrase_Wx_add_b), QA_phrase_pro),[params.qa_batch_size, params.qa_nl_phrase_max_length, params.qa_kb_phrase_max_length])))
		...
		with tf.variable_scope('QA_phrase', reuse = True):
			# QA_neg_phrase_unnormalized_score = tf.exp(tf.reshape(tf.matmul(tf.tanh(tf.matmul(QA_neg_concat_phrase_state_temp, QA_phrase_weight)+QA_phrase_bias), QA_phrase_pro),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_phrase_max_length, params.qa_kb_phrase_max_length]))
                            # 将数据限制在较小的范围内！将tf.tanh()换成tf.sigmoid(),再于最外围套上一个tf.sigmoid()
                            # QA_neg_phrase_unnormalized_score = tf.sigmoid(tf.exp(tf.reshape(tf.matmul(tf.sigmoid(tf.matmul(QA_neg_concat_phrase_state_temp, QA_phrase_weight)+QA_phrase_bias), QA_phrase_pro),[params.qa_neg_size,params.qa_batch_size, params.qa_nl_phrase_max_length, params.qa_kb_phrase_max_length])))
			...
			for b in range(loader.qa_batch_num):
				...
				d = {QA_NL_character_train_inputs: now_batch[0],
					...
					QA_KB_neg_phrase_mask: now_batch[31]}
                                ...
                                """
				un, nom, nl_state, kb_state, nl_train_embed_no_oov, nl_oov_embed, kb_embed, kb_oov_embed, wo_embed = sess.run([ QA_phrase_unnormalized_score, QA_phrase_normalization, QA_NL_phrase_state, QA_KB_phrase_state, QA_NL_phrase_train_embedding_no_oov, QA_NL_oov_phrase_train_embedding, QA_KB_phrase_train_embedding, QA_KB_oov_phrase_train_embedding, NL_word_embeddings], feed_dict = d)
                                """
				...
				# sleep(5)
                                ...
                                """
                                w = sess.run(NL_character_embeddings, feed_dict = d)
				...
				print "\n"*10
                                ...
                                print QA_phrase_normalized_score, "QA_phrase_normalized_score  "*3
                                print QA_phrase_mask, "QA_phrase_mask " * 3

                                w2 = sess.run(QA_phrase_unnormalized_score, feed_dict = d)
                                w1 = sess.run(QA_phrase_normalization, feed_dict = d)
                                w3 = sess.run(QA_phrase_normalized_score, feed_dict = d)
                                w5 = sess.run(QA_phrase_mask, feed_dict=d)
                                w6 = sess.run(QA_NL_phrase_mask_rev, feed_dict=d)
				...
				for i in range(len(un)):
				    for j in range(len(un[i])):
				        for k in range(len(un[i][j])):
				            print un[i][j][k], "\t", nom[i][j], "\t", un[i][j][k] / nom[i][j][0],  "\t", i,  "\t", j
				print "\n" * 3
				...
				w = sess.run(QA_character_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "2"*1000
				print "\n"*10

				w = sess.run(QA_neg_character_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "3"*1000
				print "\n"*10

				w =sess.run(  QA_word_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "4"*1000
				print "\n"*10
				...
				w =sess.run(  QA_neg_word_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "5"*1000
				print "\n"*10

				w =sess.run(  QA_phrase_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "6"*1000
				print "\n"*10

				w =sess.run(  QA_neg_phrase_normalized_score, feed_dict = d)
				for i in range(len(w)):
				    print w[i]
				    print "7"*1000
				print "\n"*10
				...
				#loss_all += loss_val
                                ...
                                #print oov_ph, "\n" * 4
                                #print ph, "\n"*4
                                #print sess.run(tf.reshape(neg_score, [-1]))
