...
def train_op():
	with tf.Session(graph=graph, config = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
		...
		if not os.path.exists("variable_saved_cpu_only"):
			os.makedirs("variable_saved_cpu_only")
		saver.save(sess, "variable_saved_cpu_only/variables")
...
def test_op():
	...
	correct_ans_num = 0
	with tf.Session(graph=graph, config = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
		saver.restore(sess, "variable_saved_cpu_only/variables")
		# 为了缩减时间，仅取前3个batch用于测试
		# for b in range(test_loader.qa_batch_num):
		for b in range(3):
